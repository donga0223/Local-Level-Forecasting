...
all_ribbon_data <- function(date_list, location_level = NULL, model_name ="GBQR", specific_h = NULL){
  ...
  for (i in 1:date_length){
    assign(paste0("out",i), read.csv(paste('/Users/dk29776/Dropbox/UTAustin/City-Level-Forecasting-local/', model_name, '/model_output/', location_level, '/', date_list[i], "-", model_name, '.csv', sep = "")))
  }
...
df_obs_est <- function(ribbon_data, obs_data){
  df <- obs_data %>%
    left_join(ribbon_data, by = c("location", "target_end_date")) 
  
  df_state <- df %>% 
    filter(target_end_date >= as.Date("2024-10-01"),
           hsa_nci_id == "All") 
  ...
  df1 <- df %>%
    ...
    mutate(coverage = ifelse(inc >= est_low & inc <= est_high, 1, 0),
           ...
           MAPE = abs((inc-est_median)/inc),
           MAPE.state = abs((inc - est_median.state)/inc))
...
get_wis <- function(target_date, location_level = NULL, state_level = NULL, model_name ="GBQR", filtered_obs_data){
  assign("out", read.csv(paste('/Users/dk29776/Dropbox/UTAustin/City-Level-Forecasting-local/', model_name, '/model_output/', location_level, '/', target_date, "-", model_name, '.csv', sep = "")))
  if(!is.null(state_level)){
    assign("state.out", read.csv(paste('/Users/dk29776/Dropbox/UTAustin/City-Level-Forecasting-local/', model_name, '/model_output/', state_level, '/', target_date, "-", model_name, '.csv', sep = "")))
...
forecast_plot <- function(date_list, location_level = NULL, state_level = NULL, model_name ="GBQR", 
  ...
  df_return <- df_all %>% 
    ...
    select(-target, -output_type, -no, -location.state, -hsa_nci_id.state,
           -population_hsa.state, -pop_ratio.state)
...
base_obs_data <- function(obs_data, date_var, mystate = NULL, pop_size){
  ...
  df <- obs_data %>%
    filter(population_hsa >= pop_size | hsa_nci_id == "All",
           !is.na(inc)) %>%
    mutate(target_end_date = as.Date(.data[[date_var]]),
           location = paste(state, hsa_nci_id, sep = "_")) %>%
    select(target_end_date, location, inc, state, hsa_nci_id, population_hsa, pop_ratio) %>%
    distinct()
...
obs_figure <- function(filtered_obs_data){
  state_df <- filtered_obs_data %>% 
    filter(hsa_nci_id == "All")
  
  hsa_df <- filtered_obs_data %>% 
    filter(hsa_nci_id != "All")
  
  df <- hsa_df %>%
    left_join(state_df %>%
                select(target_end_date, inc, state), 
              by = c("target_end_date", "state"),
              suffix = c("", ".state")) %>%
    mutate(week = MMWRweek(target_end_date)$MMWRweek,
           year = MMWRweek(target_end_date)$MMWRyear,
           epiweek = ifelse(week >= 30, week - 29, week + 23),
           season = ifelse(week >= 30,
                           paste0(year, "/", substr(year + 1, 3, 4)),
                           paste0(year - 1, "/", substr(year, 3, 4))),
           location = forcats::fct_reorder(location, desc(population_hsa)))
  
  
  p <- df %>%
    filter(season %in% c("2022/23", "2023/24", "2024/25")) %>%
    ggplot(aes(x = epiweek)) +
    geom_line(aes(y = inc, linetype = season, group = season), color = "orange", size = 1) +
    geom_line(aes(y = inc.state, linetype = season, group = season), color = "gray30", size = 1) +
...
cal_metrics <- function(obs_data, mystate = NULL, my_abb_state = NULL, state_level = NULL,
                        date_list, pop_size = 250000){
  if(nchar(my_abb_state) == 2){
    location_level = paste("US_NSSP_public", my_abb_state, "pct", sep = "_")
    pdf(paste("/Users/dk29776/Dropbox/UTAustin/Forecasting/Local-Level-Forecasting/each_state_figures/forecasting_", my_abb_state, ".pdf", sep = ""), width = 15, height = 10)
  }else{
    location_level = my_abb_state
    pdf(paste("/Users/dk29776/Dropbox/UTAustin/Forecasting/Local-Level-Forecasting/each_state_figures/forecasting_", my_abb_state, ".pdf", sep = ""), width = 30, height = 30)
  }
  ...
  for(i in 1:length(date_list)){
    ...
    wis[[i]] <- get_wis(target_date = date_list[i], location_level = location_level, 
                        ...
                        filtered_obs_data = filtered_obs_data)
  }
