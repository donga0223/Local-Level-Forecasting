
library(dplyr)
library(tidyverse)
library(here)
library(MMWRweek)
library(scoringutils)
library(ggplot2)


forecast_est_data <- function(out, i){
  out <- out %>% mutate(no = as.factor(i),
                        horizon = horizon + 1) 
  est_low_data <- out %>%
    filter(output_type_id == 0.025) %>%
    mutate(est_low = value) %>%
    dplyr::select(-output_type_id, -value)
  
  est_median_data <- out %>%
    filter(output_type_id == 0.5) %>%
    mutate(est_median = value) %>%
    dplyr::select(-output_type_id, -value)
  
  est_high_data <- out %>%
    filter(output_type_id == 0.975) %>%
    mutate(est_high = value) %>%
    dplyr::select(-output_type_id, -value)
  
  #ribbon_data <- out %>%
  #  filter(output_type_id %in% c(0.025, 0.5, 0.975)) %>%
  #  spread(output_type_id, value) %>%
  #  rename(est_low = `0.025`, est_high = `0.975`, est_median = `0.5`)
  
  ribbon_data <- est_low_data %>%
    left_join(est_median_data, by = c("location", "reference_date", "horizon", "target_end_date", "target", "output_type", "no")) %>%
    left_join(est_high_data, by =  c("location", "reference_date", "horizon", "target_end_date", "target", "output_type", "no")) %>%
    mutate(target_end_date = as.Date(target_end_date),
           reference_date = as.Date(reference_date))
  
  return(ribbon_data)
}


all_ribbon_data <- function(date_list, location_level = NULL, model_name ="GBQR", specific_h = NULL){
  date_length <- length(date_list)
  for (i in 1:date_length){
    assign(paste0("out",i), read.csv(paste('model_output/US_NSSP_public_Joint_250K_pct/', date_list[i], "-", model_name, '.csv', sep = "")))
  }
  
  ribbon_data <- forecast_est_data(out1, 1)
  for(j in 2:date_length){
    ribbon_data <- rbind(ribbon_data, forecast_est_data(get(paste0("out",j)), j))
  }
  
  if(!is.null(specific_h)){
    ribbon_data <- ribbon_data %>% filter(horizon == specific_h) 
  }
  
  return(ribbon_data)
}


df_obs_est <- function(ribbon_data, filtered_obs_data){
  state_inc <- filtered_obs_data %>%
    dplyr::select(target_end_date = week_end, state, inc = inc_state) %>%
    mutate(hsa_nci_id = "All",
           location = paste(state, "All", sep = "_")) %>%
    distinct() %>%
    dplyr::select(target_end_date, state, hsa_nci_id, location, inc)
  
  hsa_inc <- filtered_obs_data %>%
    dplyr::select(target_end_date = week_end, state, hsa_nci_id, inc = inc_hsa) %>%
    mutate(location = paste(state, hsa_nci_id, sep = "_"),
           hsa_nci_id = as.character(hsa_nci_id)) %>%
    dplyr::select(target_end_date, state, hsa_nci_id, location, inc)
  
  long_inc <-state_inc %>%
    bind_rows(hsa_inc)
  
  df <- long_inc %>%
    left_join(ribbon_data, by = c("location", "target_end_date")) 
  
  df_state <- df %>% 
    filter(hsa_nci_id == "All") 
  
  df1 <- df %>%
    filter(hsa_nci_id != "All") %>%
    left_join(df_state, by = c("target_end_date", "state", "reference_date", 
                               "horizon", "target", "output_type", "no"),
              suffix = c("", ".state")) %>%
    mutate(coverage = ifelse(inc >= est_low & inc <= est_high, 1, 0),
           coverage.state = ifelse(inc >= est_low.state & inc <= est_high.state, 1, 0),
           MAPE = ifelse(inc == 0, abs((inc-est_median)/((inc + est_median)/2)), abs((inc-est_median)/inc)),
           MAPE.state = ifelse(inc == 0, abs((inc - est_median.state)/((inc + est_median.state)/2)), abs((inc - est_median.state)/inc)))
  
  return(df1)
  
}


compute_wis_score <- function(df, value_col, target_date) {
  df_wis <- df %>%
    rename(predicted = !!sym(value_col),
           observed = inc,
           quantile_level = output_type_id) %>%
    dplyr::select(reference_date, state, hsa_nci_id, location, horizon, target, target_end_date,
           output_type, quantile_level, observed, predicted) |>
    as_forecast_quantile(
      forecast_unit = c("state", "hsa_nci_id", "location", "target_end_date", "horizon", "target"),
      value_col = "predicted",
      quantile_col = "quantile_level"
    ) |>
    score() |>
    mutate(horizon = horizon + 1,
           reference_date = target_date) |>
    dplyr::select(state, hsa_nci_id, location, target_end_date, horizon, reference_date, wis)
  return(df_wis)
}

get_wis <- function(target_date, location_level = NULL, state_level = NULL, model_name ="GBQR", filtered_obs_data){
  assign("out", read.csv(paste('model_output/US_NSSP_public_Joint_250K_pct/', target_date, "-", model_name, '.csv', sep = "")))
  if(!is.null(state_level)){
    assign("state.out", read.csv(paste(root, '/City-Level-Forecasting-local/', model_name, '/model_output/', state_level, '/', target_date, "-", model_name, '.csv', sep = "")))
    out = rbind(out, state.out)
  }
  out1 <- out %>%
    mutate(target_end_date = as.Date(target_end_date)) %>%
    left_join(filtered_obs_data,
              by = c("target_end_date", "location")) 
  
  out_hsa <- out1 %>% filter(hsa_nci_id != "All")
  out_state <- out1 %>% filter(hsa_nci_id == "All") 
  
  out_all <- out_hsa %>%
    left_join(out_state, by = c("reference_date", "horizon", "target", "target_end_date", 
                                "output_type", "output_type_id", "state"),
              suffix = c("", ".state"))
  
  
  wis_hsa <- compute_wis_score(out_all, "value", target_date)
  wis_state <- compute_wis_score(out_all, "value.state", target_date)
  return(list(wis_hsa = wis_hsa, wis_state = wis_state))
}

#for(i in 1:length(date_list)){
#  get_wis(target_date = date_list[i], location_level, state_level, model_name ="GBQR", filtered_obs_data)
#  print(date_list[i])
#}


forecast_plot <- function(date_list, location_level = NULL, state_level = NULL, model_name ="GBQR", 
                          specific_h = NULL, filtered_obs_data){
  
  ribbon_data = all_ribbon_data(date_list = date_list, 
                                location_level = location_level, 
                                model_name ="GBQR", 
                                specific_h = specific_h)
  if(!is.null(state_level)){
    state_ribbon_data = all_ribbon_data(date_list = date_list, 
                                        location_level = state_level, 
                                        model_name ="GBQR", 
                                        specific_h = specific_h)
    ribbon_data = rbind(ribbon_data, state_ribbon_data)
    
  }
  
  df_all <- df_obs_est(ribbon_data, filtered_obs_data)
  df_return <- df_all %>% 
    filter(!is.na(horizon)) %>%
    dplyr::select(-target, -output_type, -no, -location.state, -hsa_nci_id.state)
  
  p <- df_all %>% filter(target_end_date >= as.Date(min(date_list)-7),
                         target_end_date <= as.Date(max(date_list)+28)) %>%
    ggplot(aes(target_end_date)) + 
    geom_point(aes(y = inc, col="City Data"), size=3, shape=16, alpha=0.7) +
    geom_line(aes(y = est_median, col = "City Estimate"), size = 1) +
    geom_ribbon(aes(ymin = est_low, ymax = est_high), fill = 'orange', alpha = 0.3) +  # Add shaded ribbon
    geom_point(aes(y = inc.state, col="State Data"), size=3, shape=16, alpha=0.9) +
    geom_line(aes(y = est_median.state, col = 'State Estimate'), size = 1) +
    geom_ribbon(aes(ymin = est_low.state, ymax = est_high.state), fill = 'gray30', alpha = 0.3) +  # Add shaded ribbon
    theme_minimal() +
    scale_color_manual(
      values = c(
        "City Data" = "red",
        "City Estimate" = "orange",
        "State Data" = "black",
        "State Estimate" = "gray30"
      ),
      name = "Legend"
    ) +
    ggtitle(paste("horizon = ", specific_h)) + 
    theme(panel.spacing=unit(0, "mm"),
          legend.position = "none",
          axis.text=element_text(size=50),
          axis.title=element_text(size=60,face="bold"),
          strip.text = element_text(size = 60),
          plot.title = element_text(size = 60, face = "bold"))+
    scale_x_date(date_labels = "%b %y",  # Format dates as "Jan 01, 2023"
                 date_breaks = "2 months")+
    labs(
      x = "",  # x-axis label
      y = "Percent influenza ED visits",      # y-axis label
    ) +
    facet_wrap(state~hsa_nci_id) 
  print(p)
  
  
  p1 <- df_all %>% filter(target_end_date >= as.Date(min(date_list)-7),
                          target_end_date <= as.Date(max(date_list)+28)) %>%
    ggplot(aes(target_end_date)) + 
    geom_point(aes(y = inc, col="City Data"), size=3, shape=16, alpha=0.7) +
    geom_line(aes(y = est_median, col = "City Estimate"), size = 1) +
    geom_ribbon(aes(ymin = est_low, ymax = est_high), fill = 'orange', alpha = 0.3) +  # Add shaded ribbon
    theme_minimal() +
    #geom_point(aes(y = inc_state, col="State Data"), size=0.95, shape=1, alpha=0.9) +
    #geom_line(aes(y = est_median.state, col = 'State Estimate'), size = 1) +
    #geom_ribbon(aes(ymin = est_low.state, ymax = est_high.state), fill = 'gray30', alpha = 0.3) +  # Add shaded ribbon
    scale_color_manual(
      values = c(
        "City Data" = "red",
        "City Estimate" = "orange"
        #"State Data" = "black",
        #"State Estimate" = "gray30"
      ),
      name = "Legend"
    ) +
    ggtitle(paste("horizon = ", specific_h)) + 
    theme(panel.spacing=unit(0, "mm"),
          legend.position = "none",
          axis.text=element_text(size=50),
          axis.title=element_text(size=60,face="bold"),
          strip.text = element_text(size = 60),
          plot.title = element_text(size = 60, face = "bold"))+
    scale_x_date(date_labels = "%b %y",  # Format dates as "Jan 01, 2023"
                 date_breaks = "2 months")+
    labs(
      x = "",  # x-axis label
      y = "Percent influenza ED visits",      # y-axis label
    ) +
    facet_wrap(state~hsa_nci_id) 
  print(p1)
  
  p2 <- df_all %>% filter(target_end_date >= as.Date(min(date_list)-7),
                          target_end_date <= as.Date(max(date_list)+28)) %>%
    ggplot(aes(target_end_date)) + 
    geom_point(aes(y = inc, col="City Data"), size=3, shape=16, alpha=0.7) +
    theme_minimal() +
    #geom_line(aes(y = est_median, col = "City Estimate"), size = 1) +
    #geom_ribbon(aes(ymin = est_low, ymax = est_high), fill = 'orange', alpha = 0.3) +  # Add shaded ribbon
    #geom_point(aes(y = inc_state, col="State Data"), size=0.95, shape=1, alpha=0.9) +
    geom_line(aes(y = est_median.state, col = 'State Estimate'), size = 1) +
    geom_ribbon(aes(ymin = est_low.state, ymax = est_high.state), fill = 'gray30', alpha = 0.3) +  # Add shaded ribbon
    scale_color_manual(
      values = c(
        "City Data" = "red",
        #"City Estimate" = "orange"
        #"State Data" = "black",
        "State Estimate" = "gray30"
      ),
      name = "Legend"
    ) +
    ggtitle(paste("horizon = ", specific_h)) + 
    theme(panel.spacing=unit(0, "mm"),
          legend.position = "none",
          axis.text=element_text(size=50),
          axis.title=element_text(size=60,face="bold"),
          strip.text = element_text(size = 60),
          plot.title = element_text(size = 60, face = "bold"))+
    scale_x_date(date_labels = "%b %y",  # Format dates as "Jan 01, 2023"
                 date_breaks = "2 months")+
    labs(
      x = "",  # x-axis label
      y = "Percent influenza ED visits",      # y-axis label
    ) +
    facet_wrap(state~hsa_nci_id) 
  print(p2)
  
  return(df_return)
}


base_obs_data <- function(obs_data, date_var, mystate = NULL, pop_size){
  if (!is.null(mystate)) {
    obs_data <- obs_data %>% filter(state == mystate)
  }
  df <- obs_data %>%
    filter(population_hsa >= pop_size,
           !is.na(inc_hsa)) %>%
    mutate(target_end_date = as.Date(.data[[date_var]]),
           location = paste(state, hsa_nci_id, sep = "_")) %>%
    distinct()
  
  return(df)
}


obs_figure <- function(filtered_obs_data){
  
  p0 <- filtered_obs_data %>%
    filter(season %in% c("2022/23", "2023/24", "2024/25")) %>%
    ggplot(aes(x = target_end_date)) +
    geom_line(aes(y = inc_hsa), color = "orange", size = 1) +
    geom_line(aes(y = inc_state), color = "gray30", size = 1) +
    theme_minimal()+
    facet_wrap(state~hsa_nci_id) +
    labs(
      x = "Date",
      y = "Percent influenza ED visits",
    ) +
    #ggtitle("City vs State Trend by Season") +
    #scale_x_continuous(breaks = seq(0, 52, by = 4)) +  # 필요시 x축 조절
    theme(
      panel.spacing = unit(0, "mm"),
      legend.position = "bottom",
      axis.text = element_text(size = 50),
      axis.title = element_text(size = 60, face = "bold"),
      strip.text = element_text(size = 60),
      plot.title = element_text(size = 60, face = "bold"),
      legend.text = element_text(size = 50),
      legend.title = element_text(size = 50, face = "bold") 
    )
  print(p0)
  
  p <- filtered_obs_data %>%
    filter(season %in% c("2022/23", "2023/24", "2024/25")) %>%
    ggplot(aes(x = season_week)) +
    geom_line(aes(y = inc_hsa, linetype = season, group = season), color = "orange", size = 1.5) +
    geom_line(aes(y = inc_state, linetype = season, group = season), color = "gray30", size = 1.5) +
    theme_minimal()+
    facet_wrap(state~hsa_nci_id) +
    scale_linetype_manual(values = c("dashed", "dotted", "solid")) +  
    labs(
      x = "Epiweek",
      y = "Percent influenza ED visits",
      linetype = "Season"
    ) +
    #ggtitle("City vs State Trend by Season") +
    #scale_x_continuous(breaks = seq(0, 52, by = 4)) +  # 필요시 x축 조절
    theme(
      panel.spacing = unit(0, "mm"),
      legend.position = "bottom",
      axis.text = element_text(size = 50),
      axis.title = element_text(size = 60, face = "bold"),
      strip.text = element_text(size = 60),
      plot.title = element_text(size = 60, face = "bold"),
      legend.text = element_text(size = 50),
      legend.title = element_text(size = 50, face = "bold") 
    )
  print(p)
  
  myseasons <- c("2022/23", "2023/24", "2024/25")
  for(i in 1:length(myseasons)){
    
    p <- filtered_obs_data %>%
      filter(season %in% myseasons[i]) %>%
      ggplot(aes(x = season_week)) +
      geom_line(aes(y = inc_hsa), color = "orange", size = 1) +
      geom_line(aes(y = inc_state), color = "gray30", size = 1) +
      theme_minimal()+
      facet_wrap(state~hsa_nci_id) +
      labs(
        title = myseasons[i],
        x = "Epiweek",
        y = "Percent influenza ED visits",
        linetype = "Season"
      ) +
      #ggtitle("City vs State Trend by Season") +
      #scale_x_continuous(breaks = seq(0, 52, by = 4)) +  # 필요시 x축 조절
      theme(
        panel.spacing = unit(0, "mm"),
        axis.text = element_text(size = 50),
        axis.title = element_text(size = 60, face = "bold"),
        strip.text = element_text(size = 60),
        plot.title = element_text(size = 60, face = "bold")
      )
    print(p)
  }
  
}


#location_level = "US_top81_NSSP_public_multiCounty_pct"
## create figures and calculate metrics 
cal_metrics <- function(obs_data, mystate = NULL, my_abb_state = NULL, state_level = NULL,
                        date_list, pop_size = 250000, pdfname = NULL){
  if(nchar(my_abb_state) == 2){
    location_level = paste("US_NSSP_public", my_abb_state, "pct", sep = "_")
    pdf(paste("paper_figures/forecasting_", my_abb_state, ".pdf", sep = ""), width = 15, height = 10)
  }else if(!is.null(pdfname)){
    location_level = NULL
    pdf(paste("paper_figures/forecasting_", pdfname, ".pdf", sep = ""), width = 30, height = 30)
  }else{
    location_level = my_abb_state
    pdf(paste("paper_figures/forecasting_", my_abb_state, ".pdf", sep = ""), width = 30, height = 30)
  }
  filtered_obs_data <- base_obs_data(obs_data, date_var = "week_end", mystate = mystate, pop_size = pop_size)
  
  obs_figure(filtered_obs_data)
  h1 <- forecast_plot(date_list, location_level = location_level, state_level = state_level,
                      model_name = "GBQR", specific_h = 1, filtered_obs_data = filtered_obs_data)
  h2 <- forecast_plot(date_list, location_level = location_level, state_level = state_level,
                      model_name = "GBQR", specific_h = 2, filtered_obs_data = filtered_obs_data)
  h3 <- forecast_plot(date_list, location_level = location_level, state_level = state_level,
                      model_name = "GBQR", specific_h = 3, filtered_obs_data = filtered_obs_data)
  h4 <- forecast_plot(date_list, location_level = location_level, state_level = state_level,
                      model_name = "GBQR", specific_h = 4, filtered_obs_data = filtered_obs_data)
  dev.off()
  
  all_h <- rbind(h1,h2,h3,h4)
  
  wis <- list()
  state_inc <- filtered_obs_data %>%
    dplyr::select(target_end_date = week_end, state, inc = inc_state) %>%
    mutate(hsa_nci_id = "All",
           location = paste(state, "All", sep = "_")) %>%
    distinct() %>%
    dplyr::select(target_end_date, state, hsa_nci_id, location, inc)
  
  hsa_inc <- filtered_obs_data %>%
    dplyr::select(target_end_date = week_end, state, hsa_nci_id, inc = inc_hsa) %>%
    mutate(location = paste(state, hsa_nci_id, sep = "_"),
           hsa_nci_id = as.character(hsa_nci_id)) %>%
    dplyr::select(target_end_date, state, hsa_nci_id, location, inc)
  
  long_inc <-state_inc %>%
    bind_rows(hsa_inc)
  
  for(i in 1:length(date_list)){
    print(date_list[i])
    wis[[i]] <- get_wis(target_date = date_list[i], location_level = location_level, 
                        state_level = state_level, model_name ="GBQR", 
                        filtered_obs_data = long_inc)
  }
  wis_hsa <- map_dfr(wis, ~ .x$wis_hsa )
  wis_state <- map_dfr(wis, ~ .x$wis_state )
  
  wis_all <- wis_hsa %>%
    left_join(wis_state, 
              by = c("state", "hsa_nci_id",  "location", "target_end_date", "horizon", "reference_date"),
              suffix = c("", ".state"))
  
  df_all <- all_h %>%
    left_join(wis_all, 
              by = c("state", "hsa_nci_id",  "location", "target_end_date", "horizon", "reference_date"))
  
  return(df_all)
}

add_facet_title <- function(df_long, facet, facet_map, facet_levels) {
  
  if (!is.null(facet)) {
    df_long <- df_long %>%
      mutate(
        facet_title = facet_map[as.character(hsa_nci_id)],
        facet_title = factor(facet_title, levels = facet_levels)
      )
  }
  
  df_long
}

forecast_compare_plot <- function(df_all, date_list, pdfname = NULL, 
                                  facet = NULL, facet_map = NULL, facet_levels = NULL){
  if(!is.null(pdfname)){
    pdf(paste("paper_figures/compare_forecasting_", pdfname, ".pdf", sep = ""), width = 27, height = 13.5)
  }
  for(specific_h in 1:4){
    df_long <- df_all %>%
      filter(horizon == specific_h) %>%
      dplyr::select(target_end_date, state, hsa_nci_id, location, inc, est_median, est_low, est_high, 
             est_median.state, est_low.state, est_high.state) %>%
      pivot_longer(
        cols = c(est_median, est_low, est_high,
                 est_median.state, est_low.state, est_high.state),
        names_to = c(".value", "level"),
        names_pattern = "^(est_(?:median|low|high))(?:\\.(state))?$"
      ) %>%
      mutate(level = ifelse(level == "", "location", level)) %>%
      mutate(level = factor(level, levels = c("location","state")), # 왼쪽: local, 오른쪽: state
             location = factor(location, levels = unique(location)))
    
    #if(!is.null(facet)){
    #  df_long <- df_long %>%
    #    mutate(
    #      facet_title = case_when(
    #        #hsa_nci_id == 289 ~ "Duluth, MN",
    #        hsa_nci_id == 826 ~ "Cherokee, GA",
    #        hsa_nci_id == 865 ~ "Rockland, NY",
    #        hsa_nci_id == 170 ~ "Durham, NC",
    #        hsa_nci_id == 405 ~ "Amarillo, TX",
    #        hsa_nci_id == 434 ~ "Fort Worth, TX",
    #        hsa_nci_id == 538 ~ "Laredo, TX",   
    #        TRUE ~ NA_character_
    #      ),
    #      facet_title = fct_relevel(
    #        facet_title,
    #        "Cherokee, GA", "Rockland, NY", "Durham, NC", #"Duluth, MN", 
    #        "Amarillo, TX", "Laredo, TX", "Fort Worth, TX"
    #      )
    #    )
    #  
    #}
    if(!is.null(facet)){
      df_long <- add_facet_title(df_long, facet, facet_map, facet_levels) 
    }
    
    p <- df_long %>%
      filter(target_end_date >= as.Date(min(date_list) - 7),
             target_end_date <= as.Date(max(date_list) + 28)) %>%
      mutate(level = factor(level, levels = c("location", "state"))) %>%
      ggplot(aes(x = target_end_date)) +
      geom_point(aes(y = inc, shape = "observed HSA"), color = 'red', size = 3, alpha = 0.8) +
      geom_line(aes(y = est_median, color = level), size = 1.2) +
      geom_ribbon(aes(ymin = est_low, ymax = est_high, fill = level), alpha = 0.25, color = NA) +
      facet_grid(
        rows   = vars(level),        # 위: location, 아래: state
        cols   = vars(facet_title)  # 6개 지역
        #scales = "free_y"
      ) +
      theme_minimal() +
      scale_shape_manual(values = c("observed HSA" = 16), name = "") +
      scale_color_manual(values = c(location = "orange", state = "gray30"), 
                         breaks = c("location", "state"),
                         labels = c("predicted HSA", "predicted state"),
                         name = "") +
      scale_fill_manual(values  = c(location = "orange", state = "gray30"), 
                        breaks = c("location", "state"),
                        labels = c("predicted HSA", "predicted state"),
                        name = "") +
      guides(
        shape = guide_legend(order = 1),
        color = guide_legend(order = 2),
        fill  = guide_legend(order = 2)
      ) +
      ggtitle(paste("horizon = ", specific_h)) + 
      theme(text       = element_text(colour = "black"),
            strip.text = element_text(size = 36, colour = "black"),
            plot.title = element_text(size = 40, colour = "black"),
            legend.text  = element_text(size = 35, colour = "black"),
            
            ## facet 위/아래 간격 (기존 0 → 약간 띄우기)
            panel.spacing.y = unit(3, "lines"),
            panel.spacing.x = unit(1, "lines"),
            # 그리드 제거 
            #panel.grid = element_blank(),
            panel.grid.major.x = element_blank(),
            panel.grid.minor.x = element_blank(),
            ## y축 제목과 숫자 사이 간격
            axis.title.y = element_text(
              size = 40, colour = "black", #face = "bold", 
              margin = margin(r = 25)   # 오른쪽 여백 → 숫자와 더 띄우기
            ),
            axis.title.x = element_text(
              size   = 40,
              colour = "black",
              margin = margin(t = 25)     # 제목 위쪽 여백 (아래로 떨어져 보이게)
            ),
            axis.text.y = element_text(size = 35, colour = "black"),
            axis.text.x = element_text(size = 35, colour = "black"),
            
            ## x, y tick 은 유지 (명시적으로 써줘도 됨)
            axis.ticks.x = element_line(colour = "black", linewidth = 1.5),
            axis.ticks.y = element_line(colour = "black", linewidth = 1.5),
            axis.ticks.length = unit(0.4, "cm"),
            strip.text.y.right = element_blank(),      # ⭐ 오른쪽 strip 텍스트 제거
            strip.background.y = element_blank(),      # (선택) 오른쪽 strip 배경도 제거
            
            
            #legend.position = c(0.9, 0.4),
            #legend.position = "right",
            legend.position = "bottom",
            legend.box      = "horizontal",
            legend.direction = "horizontal",
            legend.key.height = unit(3, "cm"),
            legend.key.width = unit(2.5, "cm"),
            legend.spacing.y   = unit(0, "cm"),
            legend.box.spacing = unit(2, "cm"))+
      scale_x_date(date_labels = "%b", date_breaks = "2 months") +
      labs(x = "", y = "Percent influenza ED visits")
    
    print(p)
  }
  if(!is.null(pdfname)){
    dev.off()
  }
}
