...
all_ribbon_data <- function(date_list, location_level = NULL, model_name ="GBQR", specific_h = NULL){
  ...
  if(!is.null(location_level)){
    for (i in 1:date_length){
      assign(paste0("out",i), read.csv(paste('model_output/', location_level,'/', date_list[i], "-", model_name, '.csv', sep = "")))
    }
  }else{
    for (i in 1:date_length){
...
df_obs_est <- function(ribbon_data, filtered_obs_data){
  ...
  df1 <- df %>%
    ...
    mutate(coverage = ifelse(inc >= est_low & inc <= est_high, 1, 0),
           ...
           error = inc-est_median,
           error.state = inc - est_median.state,
           # State forecast vs State observation
           coverage.state_obs = ifelse(inc.state >= est_low.state & inc.state <= est_high.state, 
                                       1, 0),
           MAPE.state_obs = ifelse(inc.state == 0,abs(
               (inc.state - est_median.state) /((inc.state + est_median.state) / 2)),
             abs((inc.state - est_median.state) /inc.state)),
           error.state_obs = inc.state - est_median.state)
  return(df1)
}
...
get_wis <- function(target_date, location_level = NULL, state_level = NULL, model_name ="GBQR", filtered_obs_data){
  if(is.null(location_level)){
    ...
    if(!is.null(state_level)){
      assign("state.out", read.csv('model_output/US_NSSP_public_Joint_250K_pct/', target_date, "-", model_name, '.csv', sep = ""))
      out = rbind(out, state.out)
    }
  }else{
    assign("out", read.csv(paste('model_output/', location_level, '/', target_date, "-", model_name, '.csv', sep = "")))
    if(!is.null(state_level)){
      assign("state.out", read.csv('model_output/', location_level, '/', target_date, "-", model_name, '.csv', sep = ""))
      out = rbind(out, state.out)
    }
  }
  ...
  # State forecast evaluated against state observations
  out_state_eval <- out_all %>%
    transmute(
      reference_date,
      horizon,
      target,
      ...
      hsa_nci_id = hsa_nci_id.state,
      value = value.state,
      inc = inc.state
    ) %>%
    distinct()
  
  wis_state_obs <- compute_wis_score(
    out_state_eval,
    "value",
    target_date
  )
...
cal_metrics <- function(obs_data, mystate = NULL, my_abb_state = NULL, state_level = NULL,
  ...
  wis_state_obs <- map_dfr(wis, ~ .x$wis_state_obs) %>%
    rename(wis_state_obs = wis)
  ...
  wis_all <- wis_hsa %>%
    ...
    left_join(
      ...
      by = c("state", "target_end_date", "horizon", "reference_date"))
