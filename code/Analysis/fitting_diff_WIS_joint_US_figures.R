...
us_hsa_county_popdesc <- read.csv("data/us_hsa_county_popdesc.csv")

us_hsa_county_popdesc_bygroup <- us_hsa_county_popdesc %>%
  select(state, hsa_nci_id) %>% 
  distinct()
...
RMSE_list <- read.csv("data/RMSE_list.csv")
############################
...
state_abbrevs <- read.csv("data/state-abbrevs.csv")
...
RMSE_usmap <- function(df_conus, us_map, fill_var){
  p3 <- ggplot() +
...
RMSE_usmap(df_conus = df_250K_h1, us_map = us_map1, fill_var = "rmse")
...
NSSP_available_list <- NSSP_hsa %>%
  group_by(geo_value) %>%
  summarise(available_nssp = n())

NSSP_available_list_add <- us_map1 %>%
  mutate(hsa_nci_id = as.character(hsa_nci_id)) %>%
  left_join(NSSP_available_list, by = c("hsa_nci_id" = "geo_value"))


NSSP_available_list_add2 <- NSSP_available_list_add %>%
  mutate(
    fill_cat = case_when(
...
p3 <- ggplot() +
  geom_sf(
    data = sf::st_set_geometry(sf::st_as_sf(NSSP_available_list_add2), "geometry_hsa"),
    aes(fill = fill_cat),
    color = "gray20"
  ) +
  geom_sf(
...
ggsave(paste0("paper_figures/available_nssp.png"),
       p3, width = 10, height = 5, dpi = 300)
...
wis_diff_hat_usmap <- function(df_conus, us_map, model_name, target_var, fill_var, target_name_var = NULL){
  ...
  if(!is.null(target_name_var)){
    p4 <- p3 +
      scale_fill_viridis_c(option = "magma", direction = -1,na.value = "white",
                           name = expression("Predicted " * Delta * " MWIS"),
...
fit_predict_us_map <- function(gam_fit, df_250K, us_map, h, model_name, fill_var, target_name_var = NULL){
  ...
  ggsave(paste0("paper_figures/", model_name, "_", h,"_USmap.png"),
...
fit_predict_us_map(gam_fit = lm_res1, df_250K = df_250K_h1, h = 1, 
                   us_map = us_map1,
                   model_name = "LM2_log_wis_log_var3_interaction_nmsa",
